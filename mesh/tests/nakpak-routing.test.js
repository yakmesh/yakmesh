...
describe('NakpakLayer', () => {
  ...
  describe('Key Encapsulation', () => {
    ...
    beforeEach(async () => {
      ...
      layer2 = new NakpakLayer({ hopIndex: 0, nodeId: 'receiver' });
    ...
    test('shared secrets match after key exchange', async () => {
      const receiverPubKey = bytesToHex(layer2.kemKeyPair.publicKey);
      const { ciphertext } = layer1.encapsulateKey(receiverPubKey);
      layer2.decapsulateKey(ciphertext);

      // Both should derive same encryption key from same shared secret
      expect(bytesToHex(layer1.encryptionKey)).toBe(bytesToHex(layer2.encryptionKey));
    });
  ...
  describe('Layer Encryption/Decryption', () => {
    let sender, receiver;

    beforeEach(async () => {
      sender = new NakpakLayer({ hopIndex: 0, nodeId: 'sender' });
      receiver = new NakpakLayer({ hopIndex: 0, nodeId: 'receiver' });
