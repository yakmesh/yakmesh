...
export class GumbaProof {
  ...
  static createAttestation(options) {
    const {
      ...
      granteeDokoId,
      grantorSecretKey,
      expiry = aguwa.now() + GUMBA_CONFIG.attestationMaxAge,
  ...
  static verifyAttestation(attestation, grantorPublicKey, memberTree) {
    // Check expiry
    if (aguwa.now() > attestation.expiry) {
      return { valid: false, reason: 'EXPIRED' };
    }
...
export class GumbaGate {
  ...
  _checkCache(proof) {
    ...
    if (cached && cached.expiresAt > aguwa.now()) {
      return cached;
    }
  ...
  _cacheProof(proof, result) {
    ...
    this.proofCache.set(dokoId, {
      dokoId,
      role: result.role,
      expiresAt: aguwa.now() + GUMBA_CONFIG.proofCacheTime,
    });
  }
...
export class GumbaHub extends EventEmitter {
  ...
  async getMessages(sessionId, options = {}) {
    ...
    if (aguwa.now() > session.expiresAt) {
