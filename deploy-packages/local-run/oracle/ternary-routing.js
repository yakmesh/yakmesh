/**
 * TERNARY ROUTING — 162-Trit Hierarchical Mesh Addressing
 * 
 * A 3-tier hierarchical addressing system using 162 balanced ternary digits,
 * structured as 3 × 54-trit segments (6 × 9-trit sub-blocks per tier).
 * 
 * Why 162 trits?
 * - 162 × log₂(3) ≈ 256.8 bits — TRUE 256-bit post-quantum security
 * - 162 = 2 × 3⁴ — clean ternary factorization
 ...
 * - Grover's algorithm halves binary bit-security but gains NO advantage
 *   over balanced ternary — ternary search is already optimal
 * - Result: 162T provides true 256-bit equivalent post-quantum routing security
 * - Combined with ML-DSA-65 signatures and ML-KEM-768 key exchange,
 *   the entire mesh addressing layer is quantum-resistant
 * 
 * Hierarchy (3 tiers, 54 trits each):
 * 
 *   ┌──────────────────────────────────────────────────────────────────┐
 *   │ Summit (54 trits) │ Ridge (54 trits)  │ Base (54 trits)        │
 *   │    ≈ 85.6 bits    │    ≈ 85.6 bits    │    ≈ 85.6 bits         │
 *   └──────────────────────────────────────────────────────────────────┘
 * 
 * Within each tier, 54 trits decompose into 6 × 9-trit sub-blocks:
 *   [region:9] [sector:9] [cell:9] [local:9] [ident:9] [micro:9]
 * 
 * This maps naturally to the SST hexagonal tessellation (HexCoord):
 *   - 9 trits = 3^9 = 19683 addresses per sub-block
 *   - 6 sub-blocks per tier = 3^54 ≈ 5.8 × 10^25 per tier
 * 
 * Integration:
 *   TRIBHUJ (trit algebra) → ROUTING (addressing) → YPC-27 (integrity per tier)
 *   SST (hex geometry) → ROUTING (spatial mapping) → PRAHARI (entropy derivation)
 *   MANDALA (mesh topology) → ROUTING (hierarchical routing)
 * 
 * @module oracle/ternary-routing
 * @license MIT
...
/** Total trit length of a full 162T address */
export const TOTAL_TRITS = 162;

/** Number of hierarchical tiers */
export const TIER_COUNT = 3;

/** Trits per tier */
export const TRITS_PER_TIER = 54;

/** Sub-blocks per tier */
export const SUB_BLOCKS_PER_TIER = 6;
...
export const TierName = Object.freeze({
  SUMMIT: 0,  // Broadest scope — intercontinental / satellite routing
  RIDGE:  1,  // Regional mesh clusters
  BASE:   2,  // Individual node identity
});

/** Tier name labels */
export const TIER_LABELS = Object.freeze(['summit', 'ridge', 'base']);
...
/**
 * A 162-trit hierarchical mesh address.
 * Immutable value object with tier-aware operations.
 */
export class TritAddress {
  /** @type {Int8Array} — 162 balanced ternary digits */
  #trits;

  /**
   * Create a TritAddress.
   * @param {Int8Array | number[] | string} value — 162 trits or trit string
   */
  constructor(value) {
  ...
  /**
   * Get a specific tier's 54-trit segment.
   * @param {number} tier — 0-2 (SUMMIT, RIDGE, BASE)
   * @returns {Int8Array} — 54 trits
   */
  getTier(tier) {
   ...
   * @param {number} tier — 0-2
   * @param {number} subBlock — 0-5 (region, sector, cell, local, ident, micro)
   ...
   * Addresses on the same summit share tier 0.
   * Addresses on the same ridge share tiers 0+1.
   * 
   * @param {TritAddress} other 
   * @param {number} tier — check up to and including this tier (0-2)
   * @returns {boolean}
   */
   ...
   * Distance 1 = different base, same ridge
   * Distance 2 = different ridge, same summit
   * Distance 3 = different summit
   * 
   * @param {TritAddress} other 
   * @returns {number} — 0-3
   */
   ...
   * Format: Summit.Ridge.Base (each tier as trit string)
   ...
   * Converts hex bytes → trits (5 trits per byte), pads/truncates to 162.
   * 
  ...
  static fromHex(hexId) {
    ...
    // Pad or truncate to exactly 162
   ...
   * @param {Int8Array} summit — 54 trits
   * @param {Int8Array} ridge — 54 trits
   * @param {Int8Array} base — 54 trits
   * @returns {TritAddress}
   */
  static fromTiers(summit, ridge, base) {
    const trits = new Int8Array(TOTAL_TRITS);
    trits.set(summit, 0);
    trits.set(ridge, TRITS_PER_TIER);
    trits.set(base, TRITS_PER_TIER * 2);
    return new TritAddress(trits);
  }
...
/**
 * TernaryRoutingTable — XOR-metric routing for 162T addresses.
 * 
 ...
 * peers by tier distance (summit, ridge, base).
 * 
 * Routing priority:
 * 1. Same ridge (tier distance 1) — direct connection preferred
 * 2. Same summit (distance 2) — relay via ridge peer
 * 3. Different summit (distance 3) — relay via summit peer
 */
export class TernaryRoutingTable {
  /**
   * @param {TritAddress} selfAddress — this node's 162T address
  ...
  constructor(selfAddress, bucketSize = 6) {
    ...
    // 3 buckets — one per tier distance (1=base, 2=ridge, 3=summit)
   ...
   * @param {TritAddress} peerAddress — peer's 162T address
...
/**
 * Convert a standard 64-char hex node ID into a 162T address with
 ...
 * @param {number} [locality.summit=0] — summit index (0 for default)
 * @param {number} [locality.ridge=0] — ridge index
 * @returns {TritAddress}
 */
export function hexIdToAddress(hexId, locality = {}) {
  // Convert hex to raw trits
  const bytes = [];
  const clean = hexId.replace(/^0x/, '').slice(0, 66); // Max 33 bytes = 165 trits (≥162)
  ...
  // Summit tier: use locality hint or derive from first bytes
  if (locality.summit !== undefined) {
    const summitTrits = new TritArray(locality.summit, TRITS_PER_TIER).toJSON();
    for (let i = 0; i < TRITS_PER_TIER && i < summitTrits.length; i++) {
      trits[i] = summitTrits[i];
    }
  } else {
  ...
  // Ridge tier: derive or use hint
  if (locality.ridge !== undefined) {
    const ridgeTrits = new TritArray(locality.ridge, TRITS_PER_TIER).toJSON();
    for (let i = 0; i < TRITS_PER_TIER && i < ridgeTrits.length; i++) {
      trits[TRITS_PER_TIER + i] = ridgeTrits[i];
    }
  } else {
    for (let i = 0; i < TRITS_PER_TIER; i++) {
  ...
  // Base tier: always derived from hex ID (unique node portion)
  for (let i = 0; i < TRITS_PER_TIER; i++) {
...
/**
 * Verify the integrity of a 162T address using YPC-27.
 * Computes a checksum of all 3 tiers and compares.
 * 
 * @param {TritAddress} address 
