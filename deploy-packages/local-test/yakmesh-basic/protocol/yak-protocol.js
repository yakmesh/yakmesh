...
/**
 * Default node port — read from config/ports.json (single source of truth)
 * Override via YAKMESH_HTTP_PORT env var for multi-node setups
 */
let DEFAULT_PORT = 3080;
try {
  const _ports = JSON.parse(readFileSync(join(__dirname, '..', 'config', 'ports.json'), 'utf8'));
  DEFAULT_PORT = parseInt(process.env.YAKMESH_HTTP_PORT, 10) || _ports.httpPort || 3080;
} catch { DEFAULT_PORT = parseInt(process.env.YAKMESH_HTTP_PORT, 10) || 3080; }
...
try {
  ...
  PORT = parseInt(process.env.YAKMESH_HTTP_PORT, 10) || portsJson.httpPort || ${this.port};
} catch { PORT = parseInt(process.env.YAKMESH_HTTP_PORT, 10) || ${this.port}; }
...
`;
    ...
    try {
      execFileSync('reg', ['import', regPath], { stdio: 'pipe' });
    } catch (e) {
  ...
  async _unregisterWindows() {
    try {
      execFileSync('reg', ['delete', 'HKCU\\Software\\Classes\\yak', '/f'], { stdio: 'pipe' });
    } catch (e) {
      // Key might not exist
    }
  }
  ...
  async _unregisterMacOS() {
    ...
    try {
      if (existsSync(plistPath)) {
        execFileSync('rm', [plistPath], { stdio: 'pipe' });
      }
    } catch (e) {
      // Ignore
    }
...
`;
    ...
    try {
      execFileSync('xdg-mime', ['default', 'yakmesh-protocol.desktop', 'x-scheme-handler/y'], { stdio: 'pipe' });
  ...
  async _unregisterLinux() {
    ...
    try {
      if (existsSync(desktopPath)) {
        execFileSync('rm', [desktopPath], { stdio: 'pipe' });
      }
    } catch (e) {
      // Ignore
    }
